...
function normalizeStats(stats) {
  return _.mapValues(stats.assetsByChunkName, assets => {
    if (!_.isArray(assets)) {
      assets = [assets]
    }

    return assets.filter(asset => !asset.match(/\.map$/))
  })
}
...
export default function makePluginData(store, pathPrefix) {
  ...
  const manifest = normalizeStats(stats)
