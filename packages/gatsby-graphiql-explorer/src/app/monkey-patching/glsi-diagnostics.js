...
console.log(`custom diagnostics`)
...
function validateQuery(ast) {
  ...
  var deprecationWarningAnnotations = !_graphql.findDeprecatedUsages
    ? []
    : mapCat((0, _graphql.findDeprecatedUsages)(schema, ast), function(error) {
  ...
  return error.nodes.map(function(node) {
    ...
    return {
      ...
      range: new _graphqlLanguageServiceUtils.Range(
        new _graphqlLanguageServiceUtils.Position(loc.line - 1, loc.column - 1),
        new _graphqlLanguageServiceUtils.Position(loc.line - 1, end)
      ),
    }
  })
}
