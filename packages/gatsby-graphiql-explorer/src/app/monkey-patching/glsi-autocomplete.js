...
console.log(`custom autocomplete`)
...
// CHANGE REQUIRE PATH FROM LOCAL
...
function getAutocompleteSuggestions(schema, queryText, cursor, contextToken) {
  /*
  CUSTOM-START - appending all gatsby fragments
  */
  if (window.___fragments) {
    queryText += `\n` + Object.values(window.___fragments).join(`\n`)
  }
  /*
  CUSTOM-END
  */
...
function getSuggestionsForFieldNames(token, typeInfo, schema, queryText) {
  if (typeInfo.parentType) {
    var parentType = typeInfo.parentType
    var fields =
      parentType.getFields instanceof Function
        ? (0, _autocompleteUtils.objectValues)(parentType.getFields())
        : []
    ...
    /*
    CUSTOM-START - displaying spreading fragments without `...` in front
    */
    ...
    var relevantFrags = fragments.filter(function(frag) {
      return (
        ...
        (0, _graphql.isCompositeType)(typeMap[frag.typeCondition.name.value]) &&
...
function getSuggestionsForFragmentTypeConditions(token, typeInfo, schema) {
  ...
  if (typeInfo.parentType) {
    ...
    possibleTypes = (0, _autocompleteUtils.objectValues)(typeMap).filter(
      _graphql.isCompositeType
    )
  }
  return (0, _autocompleteUtils.hintList)(
    token,
...
function getSuggestionsForVariableDefinition(token, schema) {
  ...
  var inputTypes = (0, _autocompleteUtils.objectValues)(inputTypeMap).filter(
    _graphql.isInputType
  )
  return (0, _autocompleteUtils.hintList)(
    token,
          ...
          switch (state.prevState.kind) {
            ...
            case `AliasedField`:
              ...
              var field = parentType
                ? (0, _autocompleteUtils.getFieldDef)(schema, parentType, name)
                : null
              if (!field) {
                argDefs = null
                break
      ...
      case `EnumValue`:
        ...
        enumValue =
          enumType instanceof _graphql.GraphQLEnumType
            ? find(enumType.getValues(), function(val) {
